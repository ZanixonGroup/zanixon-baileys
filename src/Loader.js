...
exports.loadCommands = (dirCmd, log = true) => {
    ...
    try {
        ...
        log ? console.log("Starting to load all commands...".info) : null;
        let cmdCount = 0;
        files.forEach((file) => {
    ...
    zn.set("type", listType, null, "commands", true)
    Commands.list = zn.get("list", null, "commands", true);
    Commands.type = zn.get("type", null, "commands", true);
    return Commands;
}
...
exports.loadPlugins = (dirPlugins, log = true) => {
    ...
    try {
        let files = glob.sync(`${dir}/**/*.js`);
        log ? console.log("Starting to load all plugins...".info) : null;
        let pluginCount = 0;
        files.forEach((file) => {
