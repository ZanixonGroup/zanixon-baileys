...
async function start() { 
    Commands = await loadCommands("commands", false);
    Plugins = await loadPlugins("plugins", false);
    		...
    		if(id.status == "offer") {
    			if(id.isGroup == false) {
    				await zxn.sendMessage(id.from, {
    					text: `⚠️︱Call rejected automaticaly`, 
							mentions: [id.from]
    				});
...
try {
  ...
  let date = `${moment(m.timestamps).tz("Asia/Jakarta").format("HH:mm:ss")}-${moment(m.timestamps).tz("Asia/Jakarta").format("DD_MM_YYYY")}`;
  fs.writeFileSync(`./logging/error-${date}.txt`, e);
